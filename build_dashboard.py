...
/* ── main grid ── */
.grid{display:grid;grid-template-columns:2fr 1.1fr;gap:14px;padding:16px 36px 0}
...
<div class="grid">
  ...
  <div class="panel">
    <div class="panel-title">Distribution (log scale)</div>
    <canvas id="hist-chart" height="340"></canvas>
  </div>
</div>
...
function buildHistChart(rows) {
  const vals = rows.map(r => r.disk_gb).filter(v => v > 0);
  // log10 bin it manually into ~25 buckets between 0 and 5 (1B–10TB)
  const minLog = 0, maxLog = 5, nBins = 25;
  const binW = (maxLog - minLog) / nBins;
  const counts = new Array(nBins).fill(0);
  vals.forEach(v => {
    const b = Math.min(Math.floor((Math.log10(v) - minLog) / binW), nBins - 1);
    if (b >= 0) counts[b]++;
  });
  const xlabels = Array.from({length: nBins}, (_, i) => {
    const gb = Math.pow(10, minLog + i * binW);
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          titleColor: C.blue, bodyColor: C.muted,
        }
      },
      scales: {
        x: { grid:{display:false}, ticks:{color:C.muted, maxRotation:45} },
        y: { grid:{color:C.border}, ticks:{color:C.muted},
             title:{display:true, text:'Users', color:C.muted} }
      }
    }
  });
}
...
function render() {
  ...
  buildHistChart(ALL_ROWS); // histogram always uses full dataset
}
